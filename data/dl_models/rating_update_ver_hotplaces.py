...
def main():
    ...
    try:
        ...
        # extact
        df = extract_hotplaces_review(cursor)
        ...
        # log
        logging.info('############## Success To Update Hotplaces Star Rating')
...
def extract_hotplaces_review(cursor):
    # db connect
    review_cnt = cursor.execute('select hotplaces_id, review from hotplaces_review;')
    if review_cnt == 0:
    ...
    df = df.rename(columns={0: 'hotplaces_id', 1: 'review'})
    return df
...
def calculate_star_rating(df, okt, stopwords, tokenizer, loaded_model):
    ...
    grouping = df.groupby('hotplaces_id')['prediction'].mean() * 5
...
def load_mysql(grouping, cursor):
    ...
    try:
        for idx in grouping.index:
            hotplaces_id = grouping.loc[idx, 'hotplaces_id']
            star_rating = grouping.loc[idx, 'prediction']
            sql = f"UPDATE hotplaces SET star_rating = {star_rating} WHERE id = {hotplaces_id};"
            logging.info(sql)
            cursor.execute(sql)
